		...
		for (const d of f) {
			...
			if (y === e.element) {
				if (t) {
					...
					const v = H(e.id);
					if (!v) return;
					const g = ve(e), w = v.defaultLayoutDeferred ? We(g) : v.layout, L = X({
						layout: Ut({
							group: e,
							nextGroupSize: b,
...
function qt({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: u, id: a, onLayoutChange: r, onLayoutChanged: l, orientation: c = "horizontal", resizeTargetMinimumSize: m = {
	...
	}), h = Le(a), y = (0, import_react.useRef)(null), [b, v] = Kt(), g = (0, import_react.useRef)({
	...
	const E = se((x, P) => {
		...
		if (M) {
			let D = !1;
			switch (I.state) {
				case "active": D = I.hitRegions.some((U) => U.group === R);
			}
			return {
		...
		updateSeparatorProps: (x, { disabled: P, disableDoubleClick: I }) => {
			const M = g.current.separators.find((D) => D.id === x);
...
function Zt(e, t) {
	const { id: n } = Ee(), o = (0, import_react.useRef)({
		collapse: ye,
		expand: ye,
		getSize: () => ({
			asPercentage: 0,
...
function Qt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: u, groupResizeBehavior: a = "preserve-relative-size", id: r, maxSize: l = "100%", minSize: c = "0%", onResize: m, panelRef: p, style: S, ...z }) {
	const f = !!r, d = Le(r), h = Re({ disabled: s }), y = (0, import_react.useRef)(null), b = Ce(y, u), { getPanelStyles: v, id: g, orientation: w, registerPanel: E, updatePanelProps: L } = Ee(), G = m !== null, N = se((R, M, D) => {
		m?.(R, r, D);
	});
	q(() => {
		const R = y.current;
...
function nn({ children: e, className: t, disabled: n, disableDoubleClick: o, elementRef: i, id: s, style: u, ...a }) {
	...
	}), [c, m] = (0, import_react.useState)({}), [p, S] = (0, import_react.useState)("inactive"), [z, f] = (0, import_react.useState)(!1), d = (0, import_react.useRef)(null), h = Ce(d, i), { disableCursor: y, id: b, orientation: v, registerSeparator: g, updateSeparatorProps: w } = Ee(), E = v === "horizontal" ? "vertical" : "horizontal";
	q(() => {
		const N = d.current;
		if (N !== null) {
			const x = {
				...
				S(M.next.state !== "inactive" && M.next.hitRegions.some((D) => D.separator === x) ? M.next.state : "inactive");
			}), R = Pe(b, (M) => {
